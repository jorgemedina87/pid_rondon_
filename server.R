...
server <- function(input, output, session) {
  ...
  output$image_n_21 <- renderUI({
    req(credentials()$user_auth)
    
    HTML('<center><img src="https://github.com/jorgemedina87/proyecto/raw/master/www/reserva.PNG" width="600"></center>')
    
  })
  
  
  #source("modules/server_perfil.R", local = TRUE)
  source("modules/shiny_authentication.R", local = TRUE)
